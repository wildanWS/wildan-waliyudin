...
def hapusUser(users, account, trasaksi):
    print('Daftar user :')
    ...
    while True:
        ...
        for u in range(len(users)):
            for acc in account:
                if users[u]['user'] == user and acc['user']== user:
                    users.pop(u)
                    account.remove(acc)
                    write_file('user.txt', users)
                    write_file('account.txt', account)
                    trans = copy.copy(trasaksi)
                    for tran in trans:
                        if tran['user']== user:
                            trasaksi.remove(tran)
                            write_file('transaksi.txt', trasaksi)  
